    <!-- ---------------------------->
             <!-- FOOTER -->
    <!-- ---------------------------->
  
    <!-- The content of your page would go here. -->

    <footer class="footer-distributed">

      <div class="footer-left">
        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d487.6615582084915!2d-58.653327127042324!3d-34.690476331716184!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x95bcc0d6cc00ae4b%3A0xe50041e82a15ccf9!2sOnofre%20Betbeder%203896%2C%20B1712NJN%20Castelar%2C%20Provincia%20de%20Buenos%20Aires!5e0!3m2!1ses-419!2sar!4v1606571093579!5m2!1ses-419!2sar" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0" ></iframe>
      </div>

      <div class="footer-center">

        <div>
          <i class="fa fa-map-marker"></i>
          <p><span>Onofre Betbeder 3896</span>Castelar, Buenos Aires</p>
        </div>

        <div>
          <i class="fa fa-phone"></i>
          <p>+54 9 11 5855-3528</p>
        </div>

        <div>
          <i class="fa fa-envelope"></i>
          <p><span class="__cf_email__">conferro@live.com.ar</span></p>
        </div>

      </div>

      <div class="footer-right">

        <p class="footer-company-about">
          <span>CONFERRO MUEBLES</span>
          En CONERRO, usted encontrará una empresa seria y 
          honesta que le brindará una atención personalizada
        </p>

        <div class="footer-icons">

          <a href="https://www.facebook.com/Conferro-Muebles-146303112206904" target="_blank"><i class="fa fa-facebook"></i></a>
          <a href="https://www.instagram.com/conferromuebles/" target="_blank"><i class="fa fa-instagram"></i></a>
          <a href="https://wa.me/541158553528" target="_blank"><i class="fa fa-whatsapp"></i></a>

        </div>

      </div>

    </footer>
